...
async function persistAgentBarTurn(row) {
    ...
    const systemPrompt = row.systemPrompt != null && String(row.systemPrompt).trim() !== ""
    ...
    await db.transaction(async (tx) => {
        await tx.insert(agentSessions).values({
            ...
            chartPng: pngBuf,
            chartCaptureError: row.chartCaptureError != null && String(row.chartCaptureError).trim() !== ""
                ? String(row.chartCaptureError)
                : null,
            assistantText: row.assistantText != null ? String(row.assistantText) : null,
            cardSummary: row.cardSummary != null && String(row.cardSummary).trim() !== "" ? String(row.cardSummary) : null,
            toolTraceJson: encToolTrace,
            exchangeAfterJson: encAfter,
            agentOk: row.agentOk !== false ? 1 : 0,
            agentError: row.agentError != null && String(row.agentError).trim() !== "" ? String(row.agentError) : null,
            estimatedPromptTokens: null,
            contextWindowTokens: null,
            updatedAt,
            systemPromptText: systemPrompt,
            assistantReasoningText: row.assistantReasoningText != null && String(row.assistantReasoningText).trim() !== ""
...
async function listAgentBarTurnsPage(args = {}) {
    ...
    const cursorRaw = args.cursor && typeof args.cursor === "object" && !Array.isArray(args.cursor)
        ? args.cursor
        : null;
    ...
    const cap = cursorRaw &&
        ...
        : null;
    const db = getDb();
    const hasChartSql = sql `CASE WHEN ${agentSessions.chartPng} IS NOT NULL AND LENGTH(${agentSessions.chartPng}) > 0 THEN 1 ELSE 0 END`;
    const whereClause = cap && cap.capturedAt && cap.barCloseId
        ? and(eq(agentSessions.tvSymbol, tvSymbol), eq(agentSessions.interval, interval), or(lt(agentSessions.capturedAt, cap.capturedAt), and(eq(agentSessions.capturedAt, cap.capturedAt), lt(agentSessions.barCloseId, cap.barCloseId))))
    ...
    const rows = raw.map((r) => {
        ...
        return {
            ...
            assistantReasoningText: r.assistantReasoningText != null && String(r.assistantReasoningText).trim()
                ? String(r.assistantReasoningText).trim()
                : null,
            assistantDecision: r.assistantDecision != null && String(r.assistantDecision).trim() ? String(r.assistantDecision) : null,
    ...
    const nextCursor = hasMore && tail ? { capturedAt: tail.capturedAt, barCloseId: tail.barCloseId } : null;
...
async function getAgentSessionMessages(barCloseId) {
    const id = String(barCloseId || "").trim();
    if (!id)
        return { messages: [], assistantReasoningText: null, assistantDecision: null };
    const db = getDb();
...
function holdingStateFromExchangeAfter(exchangeAfter) {
    ...
    if (!pos || typeof pos !== "object" || pos.hasPosition !== true)
        return "空仓";
    if (pos.posSide === "long")
        return "持多";
...
async function listRecentAgentMemories(args = {}) {
    const tvSymbol = String(args.tvSymbol || "").trim();
    const interval = String(args.interval || "").trim();
...
export { persistAgentBarTurn, listAgentBarTurnsPage, getAgentBarTurnChart, getAgentSessionMessages, listRecentAgentMemories, redactContentForStorage, };
//# sourceMappingURL=agent-bar-turns-store.js.map