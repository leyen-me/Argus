 ...
 */
async function resolvePromptStrategyId(preferred) {
    await promptStrategiesStore.seedFromDiskIfEmpty();
    ...
    if (raw && available.includes(raw))
        return raw;
    if (available.includes(DEFAULT_PROMPT_STRATEGY))
 ...
 */
async function loadSystemPromptsFromDisk(strategyId) {
    const id = await resolvePromptStrategyId(strategyId);
    return {
        systemPromptCrypto: normalizeSystemPromptField(await promptStrategiesStore.getStrategyBody(id), EMPTY_SYSTEM_PROMPT_FALLBACK),
    };
}
async function defaultConfigFallback() {
    const promptStrategy = await resolvePromptStrategyId(typeof APP_SETTINGS_SEED.promptStrategy === "string" ? APP_SETTINGS_SEED.promptStrategy : undefined);
    return {
        ...APP_SETTINGS_SEED,
        promptStrategy,
        promptStrategies: await listPromptStrategies(),
        promptStrategySelectOptions: await listPromptStrategySelectOptions(),
        promptStrategyDecisionIntervalTv: await promptStrategiesStore.getDecisionIntervalTvForStrategyId(promptStrategy),
        promptStrategyChartIndicators: await promptStrategiesStore.getChartIndicatorsForStrategyId(promptStrategy),
        promptStrategyMarketTimeframes: await promptStrategiesStore.getMarketTimeframesForStrategyId(promptStrategy),
...
function stripSystemPromptsForPersistence(cfg) {
    if (!cfg || typeof cfg !== "object")
        return cfg;
    const { systemPromptCrypto: _c, promptStrategyDecisionIntervalTv: _p, promptStrategyChartIndicators: _pci, promptStrategyMarketTimeframes: _pmt, promptStrategySelectOptions: _pso, symbols: _s, defaultSymbol: _d, ...rest } = cfg;
    return rest;
}
async function persistLoadedConfig(normalizedCfg) {
    const payload = stripSystemPromptsForPersistence(normalizedCfg);
    await localDb.kvSet(localDb.KV_NS_APP, localDb.KV_KEY_SETTINGS, `${JSON.stringify(payload, null, 2)}\n`);
}
...
async function ensurePersistedConfig() {
    await promptStrategiesStore.seedFromDiskIfEmpty();
    if (await localDb.kvHas(localDb.KV_NS_APP, localDb.KV_KEY_SETTINGS))
        return;
 ...
 */
async function buildInitialConfigFromSeed() {
...
function normalizeDashboardStatsSegments(raw) {
...
function normalizeDashboardStrategyRanges(raw) {
    ...
    for (const [strategyIdRaw, value] of Object.entries(raw)) {
        ...
        if (!strategyId || !value || typeof value !== "object" || Array.isArray(value))
            continue;
        ...
        let segments = normalizeDashboardStatsSegments(value.segments);
...
const ALLOWED_RUNTIME_STATUS = new Set(["idle", "running", "paused", "stopped"]);
 ...
 */
function normalizeStrategyRuntimeById(raw) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw))
        return {};
    const out = {};
    for (const [kRaw, v] of Object.entries(raw)) {
        const id = typeof kRaw === "string" ? kRaw.trim() : "";
        if (!id || !v || typeof v !== "object" || Array.isArray(v))
            continue;
        const row = v;
        let status = "idle";
        if (typeof row.status === "string" && ALLOWED_RUNTIME_STATUS.has(row.status)) {
            status = row.status;
        }
        let sessionId = null;
 ...
 */
function isPromptStrategyExecutionRunning(cfg) {
 ...
 */
function isDashboardStatsSessionActive(entry) {
...
function migrateStrategyRuntimeFromLegacyDashboard(promptStrategyId, ranges) {
    ...
    if (!row || !isDashboardStatsSessionActive(row))
        return out;
    const since = typeof row.statsSince === "string" && row.statsSince.trim()
        ? row.statsSince.trim()
        : "";
    out[sid] = {
        status: "running",
...
async function normalizeConfig(raw) {
    ...
    const openaiBaseUrl = normalizeOpenAiBaseUrl(typeof r.openaiBaseUrl === "string" ? r.openaiBaseUrl : base.openaiBaseUrl);
    const openaiModel = normalizeOpenAiModel(typeof r.openaiModel === "string" ? r.openaiModel : base.openaiModel);
    const openaiApiKey = typeof r.openaiApiKey === "string" ? r.openaiApiKey.trim() : base.openaiApiKey;
    const promptStrategy = await resolvePromptStrategyId(typeof r.promptStrategy === "string" && r.promptStrategy.trim()
    ...
    if (!symbols.some((s) => s.value === defaultSymbol))
        defaultSymbol = symbols[0]?.value ?? "OKX:BTCUSDT";
    ...
    if (Number.isFinite(tt) && tt > 0)
        llmRequestTimeoutMs = Math.floor(tt);
    ...
    if (r.llmReasoningEnabled === true)
        llmReasoningEnabled = true;
    else if (r.llmReasoningEnabled === false)
        llmReasoningEnabled = false;
    let barCloseAgentAutoEnabled = base.barCloseAgentAutoEnabled;
    if (r.barCloseAgentAutoEnabled === false)
        barCloseAgentAutoEnabled = false;
    else if (r.barCloseAgentAutoEnabled === true)
        barCloseAgentAutoEnabled = true;
    let tradeNotifyEmailEnabled = base.tradeNotifyEmailEnabled;
    if (r.tradeNotifyEmailEnabled === true)
        tradeNotifyEmailEnabled = true;
    else if (r.tradeNotifyEmailEnabled === false)
        tradeNotifyEmailEnabled = false;
    const smtpHost = typeof r.smtpHost === "string" && r.smtpHost.trim() ? r.smtpHost.trim() : base.smtpHost;
    let smtpPort = base.smtpPort;
    const sp = Number(r.smtpPort);
    if (Number.isFinite(sp) && sp > 0)
        smtpPort = Math.floor(sp);
    let smtpSecure = base.smtpSecure;
    if (r.smtpSecure === true)
        smtpSecure = true;
    else if (r.smtpSecure === false)
        smtpSecure = false;
    ...
    const notifyEmailTo = typeof r.notifyEmailTo === "string" ? r.notifyEmailTo.trim() : base.notifyEmailTo;
    let okxSwapTradingEnabled = base.okxSwapTradingEnabled;
    if (r.okxSwapTradingEnabled === true)
        okxSwapTradingEnabled = true;
    else if (r.okxSwapTradingEnabled === false)
        okxSwapTradingEnabled = false;
    let okxSimulated = base.okxSimulated;
    if (r.okxSimulated === true)
        okxSimulated = true;
    else if (r.okxSimulated === false)
        okxSimulated = false;
    const okxApiKey = typeof r.okxApiKey === "string" ? r.okxApiKey.trim() : base.okxApiKey;
    const okxSecretKey = typeof r.okxSecretKey === "string" ? r.okxSecretKey.trim() : base.okxSecretKey;
    const okxPassphrase = typeof r.okxPassphrase === "string" ? r.okxPassphrase.trim() : base.okxPassphrase;
    ...
    if (!hasExplicitDashboardStrategyRanges &&
        ...
        dashboardStrategyRanges[promptStrategy] = {
            ...
            segments: typeof dashboardAgentToolStatsSince === "string" && dashboardAgentToolStatsSince.trim()
                ? [{ startedAt: dashboardAgentToolStatsSince.trim(), endedAt: null }]
                : [],
        };
    }
    const activeDashboardRange = dashboardStrategyRanges[promptStrategy] && typeof dashboardStrategyRanges[promptStrategy] === "object"
        ? dashboardStrategyRanges[promptStrategy]
        : null;
    ...
    const promptStrategyDecisionIntervalTv = await promptStrategiesStore.getDecisionIntervalTvForStrategyId(promptStrategy);
    const promptStrategyChartIndicators = await promptStrategiesStore.getChartIndicatorsForStrategyId(promptStrategy);
    const promptStrategyMarketTimeframes = await promptStrategiesStore.getMarketTimeframesForStrategyId(promptStrategy);
    return {
        symbols,
        defaultSymbol,
        interval,
 ...
 */
async function saveMergedConfigPayload(payload) {
...
export { APP_SETTINGS_SEED, loadAppConfig, normalizeConfig, databasePath, configPath, userConfigPath, buildInitialConfigFromSeed, resetAppConfig, saveMergedConfigPayload, DEFAULT_OPENAI_BASE_URL, DEFAULT_OPENAI_MODEL, loadSystemPromptsFromDisk, listPromptStrategies, resolvePromptStrategyId, stripSystemPromptsForPersistence, EMPTY_SYSTEM_PROMPT_FALLBACK, isDashboardStatsSessionActive, isPromptStrategyExecutionRunning, };
