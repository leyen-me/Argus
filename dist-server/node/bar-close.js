...
import { isLlmEnabled, buildMultiTimeframeUserPrompt, buildMultimodalUserContent, buildUserTextForHistory, runTradingAgentTurn, resolveSystemPrompt, resolveTradingAgentSystemPrompt, RECENT_CANDLES_FETCH_LIMIT, mdCell, mdTable, summarizeAgentAnalysisForCard, } from "./llm.js";
import { requestChartCaptureFromBrowser } from "./chart-capture-browser-bridge.js";
import { getOkxExchangeContextForBar, isOkxExchangeContextReadyForBarAgent, describeOkxExchangeContextGateFailure, fetchRecentCandlesForTv, fetchRecentSwapPositionsHistoryForBar, } from "./okx-perp.js";
...
import { MULTI_TIMEFRAME_CAPTURE_SPECS, canonTradingViewInterval, decisionIntervalExplain, filterMultiTimeframeSpecsByMarketSelection, } from "../shared/strategy-fields.js";
/**
 ...
 */
function resolveStrategyPositionHistorySinceMs(cfg) {
 ...
 */
function describeBarCloseStrategyExecutionSkipReason(cfg) {
    const id = typeof cfg.promptStrategy === "string" ? cfg.promptStrategy.trim() : "";
    if (!id)
        return "未调用 LLM：请先在策略中心创建并选用策略。";
...
function formatOkxPositionHistoryForPrompt(ph) {
    ...
    if (!ph.rows || ph.rows.length === 0) {
        const sinceIso = ph.sessionSinceMs != null && Number.isFinite(ph.sessionSinceMs)
            ? new Date(ph.sessionSinceMs).toISOString()
            : null;
        const emptyHint = sinceIso != null
    ...
    const tableRows = ph.rows.map((r) => {
        ...
        const side = o.direction != null && String(o.direction) !== ""
            ? String(o.direction)
            : o.posSide != null
                ? String(o.posSide)
                : "—";
...
function formatRecentAgentMemoryBlock(memories, exchangeCtx) {
    ...
    const lines = rows.map((row, idx) => {
        ...
        const note = clipText(row?.cardSummary || row?.assistantText || row?.agentError || "", 110) || "无补充说明";
        return `${idx + 1}. ${row?.capturedAt || "—"}｜${holding}｜${action}｜${note}`;
    });
    const latest = rows[0];
    const latestAfter = latest && latest.exchangeAfter && typeof latest.exchangeAfter === "object" ? latest.exchangeAfter : null;
    const latestPos = latestAfter && latestAfter.position && typeof latestAfter.position === "object" ? latestAfter.position : null;
    const latestHadPosition = latestPos && latestPos.hasPosition === true;
    const currentPos = exchangeCtx && exchangeCtx.position && typeof exchangeCtx.position === "object" ? exchangeCtx.position : null;
...
function positionFieldsTable(fields) {
    ...
    if (!entries.length)
        return "（暂无仓位明细）";
    return mdTable(["字段", "值"], entries.map(([k, v]) => [k, v]));
}
function buildOkxContextUserText(marketText, exchangeCtx, positionsHistory, recentAgentMemories) {
    ...
    const accountBlock = mdTable(["USDT 可用权益", "ct_val", "lot_sz", "min_sz", "tick_sz", "last_px"], [
        [
    ...
    const sizingBlock = sizingExamples.length > 0
        ? mdTable(["保证金占用比例", "杠杆", "预估张数", "满足最小张数"], sizingExamples.map((r) => [
            r.margin_fraction,
            r.leverage,
    ...
    const posSummary = mdTable(["有持仓", "posSide", "posNum", "absContracts"], [
        [
    ...
    return [
        ...
        "",
        algoBlock,
        formatOkxPositionHistoryForPrompt(exchangeCtx?.ok === true && exchangeCtx?.enabled === true ? positionsHistory ?? null : null),
    ].join("\n");
}
...
async function requestChartCapture(tvSymbol, marketTimeframes) {
    try {
...
async function emitBarClose(ctx) {
    ...
    const multiCaptureSpecs = filterMultiTimeframeSpecsByMarketSelection(MULTI_TIMEFRAME_CAPTURE_SPECS, marketTfs);
    let chartImage = null;
    let chartImages = [];
    ...
    const [exchangeCtx, recentCandlesPack, positionsHistory, recentAgentMemories] = await Promise.all([
        ...
        Promise.all(multiCaptureSpecs.map(async (spec) => [
            spec.interval,
    ...
    const textForLlm = buildMultiTimeframeUserPrompt(ctx.tvSymbol, ctx.periodLabel, ctx.candle, recentCandles, marketTfs, strategyIndicators);
    ...
    const intervalSkipReason = ctxIvCanon !== decisionIvCanon
    ...
    const autoAgentSkipReason = cfg.barCloseAgentAutoEnabled === false ? "未调用 LLM：右侧面板已关闭「K 线收盘自动 Agent」。" : null;
    ...
    const shouldAttemptChartCapture = llm.enabled &&
        !intervalSkipReason &&
        !exchangeSkipReason &&
        !autoAgentSkipReason &&
        !execSkipReason &&
    ...
    const orderedChartImages = multiCaptureSpecs
        ...
        .filter(Boolean);
    const hasExpectedMultiChartImages = multiCaptureSpecs.length > 0 ? orderedChartImages.length === multiCaptureSpecs.length : true;
    if (!chartCaptureError && multiCaptureSpecs.length > 0 && !hasExpectedMultiChartImages) {
        chartCaptureError = "多周期图表截图不完整。";
    }
    const multimodalImageInput = orderedChartImages.length > 0
        ? orderedChartImages
        : chartImage?.base64
            ? chartImage.base64
            : null;
    ...
    const canUseMultimodal = !chartCaptureError &&
    ...
    const currentUserContent = canUseMultimodal
        ? buildMultimodalUserContent(llmUserText, multimodalImageInput, chartImage?.mimeType || "image/png")
        : llmUserText;
    ...
    if (agentResult.ok) {
        ...
        const summaryP = agentResult.text && String(agentResult.text).trim()
        ...
        try {
            await persistAgentBarTurn({
                ...
                systemPrompt,
                messagesOut: agentResult.messagesOut,
                assistantReasoningText: cfg.llmReasoningEnabled === true && String(agentResult.reasoningText || "").trim()
...
export { emitBarClose };
//# sourceMappingURL=bar-close.js.map