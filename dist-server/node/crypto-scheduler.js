...
function onConfirmedBar(tvSymbol, intervalTv, candle) {
    ...
    return enqueueBarCloseTask(async () => {
        if (seenBarIds.has(candle.barKey))
            return;
        if (Number.isFinite(candleTs) && candleTs < latestQueuedBarTs) {
            send("market-status", {
...
function forceReconnect(okxSocket, reasonText) {
    if (intentionalClose || !session || ws !== okxSocket)
        return;
    ...
    try {
        if (typeof okxSocket.terminate === "function")
            okxSocket.terminate();
        else
            okxSocket.close();
    }
...
function connectOkx() {
    ...
    ws.on("open", () => {
        const openState = Number(runtime.WebSocketImpl?.OPEN ?? WebSocket.OPEN ?? 1);
        if (ws !== okxSocket || okxSocket.readyState !== openState)
            return;
        markSocketActivity();
        reconnectAttempt = 0;
        const sub = {
...
export { start, stop, okxSwapInstIdFromTv as okxInstIdFromTv, __setRuntimeForTests, __resetRuntimeForTests, };
//# sourceMappingURL=crypto-scheduler.js.map