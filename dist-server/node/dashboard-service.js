...
function pickDisplayEquityUsdt(metrics) {
    if (!metrics || typeof metrics !== "object")
        return null;
    if (Number.isFinite(metrics.adjEq) && metrics.adjEq > 0)
        return metrics.adjEq;
    if (Number.isFinite(metrics.usdtEq) && metrics.usdtEq >= 0)
        return metrics.usdtEq;
    return null;
}
/**
...
function pickMarginUsedUsdt(equityUsdt, avail, metrics) {
    if (metrics && Number.isFinite(metrics.imr) && metrics.imr >= 0)
        return metrics.imr;
...
function filterEquitySeriesBySegments(series, segments) {
    if (!Array.isArray(series) || !Array.isArray(segments) || segments.length === 0)
        return [];
...
function normalizeStatsSegments(segments) {
    if (!Array.isArray(segments))
        return [];
    return segments
        .map((seg) => {
        const startedAt = seg && typeof seg.startedAt === "string" && Number.isFinite(Date.parse(seg.startedAt.trim()))
            ? seg.startedAt.trim()
            : "";
        if (!startedAt)
            return null;
        const endedAt = seg && typeof seg.endedAt === "string" && Number.isFinite(Date.parse(seg.endedAt.trim()))
            ? seg.endedAt.trim()
            : null;
...
function isTimeInStatsSegments(ms, segments) {
...
function minStatsSegmentStartMs(segments) {
    if (!segments.length)
        return NaN;
...
async function getDashboardSnapshot(cfg) {
    ...
    const baselineEquityUsdt = typeof baselineRaw === "number" && Number.isFinite(baselineRaw) && baselineRaw >= 0
        ? baselineRaw
        : null;
    ...
    const dashboardAgentToolStatsSince = typeof statsSinceRaw === "string" && statsSinceRaw.trim() && Number.isFinite(Date.parse(statsSinceRaw.trim()))
        ? statsSinceRaw.trim()
        : null;
    ...
    try {
        ...
        const pnlEquityUsdt = statsSegments.length > 0 && statsSegments[statsSegments.length - 1]?.endedAt ? statsEquityTail : equityUsdt;
...
export { BACKGROUND_EQUITY_SAMPLE_INTERVAL_MS, buildEquitySeriesForDashboard, equitySamplePullLimit, filterEquitySeriesBySegments, getDashboardSnapshot, sampleDashboardEquityOnce, };
