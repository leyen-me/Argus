...
import { filterMultiTimeframeSpecsByMarketSelection, orderStrategyIndicatorsForPrompt, STRATEGY_INDICATOR_ORDER, } from "../shared/strategy-fields.js";
...
const TRADING_AGENT_INPUT_CONTEXT_APPEND = "### 本轮输入形式\n\n" +
...
function resolveSystemPrompt(cfg) {
    if (!cfg || typeof cfg.systemPromptCrypto !== "string")
        return "";
    return cfg.systemPromptCrypto;
}
/** 程序在策略正文后追加的工具与执行段落。 */
function buildTradingAgentStaticToolsAppend() {
    if (!TRADING_AGENT_TOOLS_POLICY_BLOCK)
        return "";
...
function llmRetryBaseDelayMs(cfg) {
    ...
    if (!Number.isFinite(ms) || ms <= 0)
        ms = DEFAULT_LLM_RETRY_BASE_DELAY_MS;
...
function llmRetryCountFromError(e) {
    if (!e || typeof e !== "object" || !("__argusRetryCount" in e))
        return 0;
...
function isRetryableLlmError(e) {
    ...
    const code = e && typeof e === "object" && "code" in e && e.code != null ? String(e.code).toUpperCase() : "";
    if (code === "ETIMEDOUT" ||
        code === "ECONNRESET" ||
        code === "ECONNREFUSED" ||
        code === "EPIPE" ||
...
async function withLlmRetries(cfg, run, opts = {}) {
    const maxRetries = llmMaxRetries(cfg);
    let lastError = new Error("LLM 请求失败：未知错误");
    let retryCount = 0;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!canRetry)
                throw withLlmRetryCount(e, retryCount);
            retryCount += 1;
...
function createLlmClient(apiKey, baseURL, timeout) {
    return new OpenAI({
        apiKey,
        baseURL,
        timeout,
...
function mapLlmSdkError(e, _cfg) {
    const timeoutMsg = appendLlmRetrySuffix("LLM 请求超时（可在应用设置里改 llmRequestTimeoutMs，单位毫秒）", e);
    if (e instanceof APIUserAbortError) {
...
function buildMultimodalUserContent(userText, imageBase64, mimeType) {
    if (Array.isArray(imageBase64)) {
        const images = imageBase64
            ...
            .map((item) => ({
            base64: item.base64 != null && String(item.base64).trim() !== "" ? String(item.base64).trim() : "",
            mimeType: item.mimeType != null && String(item.mimeType).trim() !== ""
...
function buildChatCompletionRequestFromMessages(messages, options, stream) {
    ...
    const model = (options.model && String(options.model).trim()) ||
        ...
        "gpt-4o-mini";
    const baseRaw = (options.baseUrl && String(options.baseUrl).trim()) ||
    ...
    const allowReasoningThisHttp = options?.llmReasoningForThisRequest !== false &&
...
function buildChatCompletionRequest(userText, options, stream) {
    const userContent = buildMultimodalUserContent(userText, options.imageBase64, options.mimeType);
    const cfg = options.appConfig;
...
function extractTradingDecision(content) {
    ...
    if (lineMatch)
        return /** @type {"hold" | "open" | "close" | "amend"} */ (lineMatch[1].toLowerCase());
    const jsonMatch = text.match(/"decision"\s*:\s*"(hold|open|close|amend)"/i);
    if (jsonMatch)
        return /** @type {"hold" | "open" | "close" | "amend"} */ (jsonMatch[1].toLowerCase());
    return null;
}
/**
 * @param {unknown} tc
    ...
    if (Array.isArray(details) && details.length > 0) {
        ...
        for (const d of details) {
            if (!d || typeof d !== "object")
                continue;
            const t = d.type;
            if (t === "reasoning.text" && typeof d.text === "string")
                s += d.text;
...
function mergeReasoningChunk(prev, piece) {
    ...
    if (!base)
        return p;
    if (p.startsWith(base) && p.length >= base.length)
        return p;
    return base + p;
}
/**
        ...
        await withLlmRetries(cfg, async () => {
            ...
            for await (const chunk of stream) {
                ...
                if (wantReasoning) {
                    ...
                    if (rawReasoning)
                        fullReasoning = mergeReasoningChunk(fullReasoning, rawReasoning);
...
async function callOpenAIChat(userText, options = {}) {
    ...
    try {
        const completion = await withLlmRetries(cfg, async () => {
            const signal = llmAbortSignal(cfg);
            return client.chat.completions.create(built.body, { signal });
        });
 ...
 */
async function runCardSummaryCompletion(messages, opts) {
...
function buildCardSummarySourceFromAgentThread(messagesOut) {
    if (!Array.isArray(messagesOut) || messagesOut.length === 0)
        return "";
    const chunks = [];
    let asstStep = 0;
    for (const m of messagesOut) {
        ...
        if (m.role === "assistant") {
            ...
            const names = Array.isArray(tcs) && tcs.length > 0
                ? tcs
            ...
            const clip = raw.length > CARD_SUMMARY_MAX_TOOL_SNIPPET_CHARS
...
async function compressCardLineForTightLimit(longLine, options, client, signalFactory) {
    ...
    if (!line)
        return "";
    if (line.length <= CARD_SUMMARY_MAX_OUT_CHARS)
        return line;
    const system = [
    ...
    try {
        ...
        const hard = (s) => s.length > CARD_SUMMARY_MAX_OUT_CHARS ? s.slice(0, CARD_SUMMARY_MAX_OUT_CHARS - 1) + "…" : s;
        if (!out)
            return hard(line);
        return hard(out);
    }
...
async function summarizeAgentAnalysisForCard(assistantFull, options, extras = {}) {
    ...
    const cut = bodyText.length > CARD_SUMMARY_MAX_INPUT_CHARS
    ...
    const signalFactory = () => typeof AbortSignal !== "undefined" && typeof AbortSignal.timeout === "function"
        ? AbortSignal.timeout(SUMMARY_LLM_MAX_MS)
        : llmAbortSignal({ llmRequestTimeoutMs: SUMMARY_LLM_MAX_MS });
    const initBuilt = buildChatCompletionRequestFromMessages([{ role: "user", content: "x" }], { ...options, llmReasoningForThisRequest: false }, false);
    if (initBuilt.error) {
        return { ok: false };
    }
    const client = createLlmClient(resolveOpenAiApiKey(cfg), initBuilt.baseURL, Math.min(SUMMARY_LLM_MAX_MS + 5_000, llmRequestTimeoutMs(cfg)));
    try {
...
function computeEmaSeries(closes, period) {
    ...
    if (!Number.isFinite(period) || period < 1 || n < period)
        return out;
    let sum = 0;
...
function computeAtrWilderSeries(tr, period) {
    const n = tr.length;
    const out = /** @type {(number | null)[]} */ (Array(n).fill(null));
    if (n < period || !Number.isFinite(period) || period < 1)
        return out;
    let sum = 0;
    for (let i = 0; i < period; i++)
        sum += tr[i];
...
function computeSuperTrendSeries(highs, lows, closes, atrPeriod, multiplier) {
    ...
    for (let i = 0; i < n; i++) {
        if (atr[i] == null || !Number.isFinite(atr[i]))
            continue;
...
function computeRsiWilderSeries(closes, period) {
    const n = closes.length;
    const out = /** @type {(number | null)[]} */ (Array(n).fill(null));
    if (!Number.isFinite(period) || period < 1 || n < period + 1)
        return out;
    const gains = [];
    const losses = [];
    for (let i = 1; i < n; i++) {
 ...
 */
function indicatorColumnHeaders(orderedIds) {
    const headers = [];
    for (const id of orderedIds) {
        ...
        if (id === "EM200")
            headers.push("E200");
        if (id === "BB")
            headers.push("BBM", "BBU", "BBL");
        if (id === "ATR")
            headers.push("ATR");
        if (id === "RSI14")
            headers.push("RSI");
        if (id === "MACD")
            headers.push("DIF", "SIG", "Hist");
        if (id === "SUPERTREND")
            headers.push("ST", "Dir");
    }
    return headers;
 ...
 * @param {readonly StrategyIndicatorId[]} orderedIds
 */
function indicatorCellsForRow(orderedIds, j, i, volumeRaw, sliceEmaById, bb, atrSeries, rsiSeries, macdTriple, stLine, stUp) {
    const cells = [];
    for (const id of orderedIds) {
        if (id === "VOL")
            cells.push(formatPromptQtyAbbrev(volumeRaw));
        if (id === "EM20" || id === "EM50" || id === "EM200") {
            const slice = sliceEmaById[id];
            cells.push(formatPromptPriceCell(slice?.[j]));
        }
        if (id === "BB") {
            cells.push(formatPromptPriceCell(bb.mid[i]), formatPromptPriceCell(bb.upper[i]), formatPromptPriceCell(bb.lower[i]));
        }
        if (id === "ATR")
            cells.push(formatPromptPriceCell(atrSeries[i]));
        if (id === "RSI14")
            cells.push(formatPromptRsiCell(rsiSeries[i]));
        if (id === "MACD") {
            cells.push(formatPromptMacdCell(macdTriple.macd[i]), formatPromptMacdCell(macdTriple.signal[i]), formatPromptMacdCell(macdTriple.hist[i]));
        }
        if (id === "SUPERTREND") {
...
function stripFixedTrailingZeros(t) {
    let s = t.replace(/\.?0+$/, "");
    if (s === "-0")
        s = "0";
    return s === "" ? "0" : s;
...
function formatPromptPriceCell(x) {
...
function formatPromptRsiCell(x) {
    if (x == null || !Number.isFinite(x))
        return "—";
    return stripFixedTrailingZeros(x.toFixed(1));
}
function formatPromptMacdCell(x) {
    if (x == null || !Number.isFinite(x))
        return "—";
    return stripFixedTrailingZeros(x.toFixed(2));
}
function formatPromptSuperTrendDir(up) {
    if (up == null)
        return "—";
    return up ? "U" : "D";
}
/** 成交量等数量字段：K/M/B 缩写（prompt 中不再输出 Turnover） */
function formatPromptQtyAbbrev(raw) {
    ...
    const ax = Math.abs(x);
    if (ax >= 1e9)
        return `${stripFixedTrailingZeros((x / 1e9).toFixed(2))}B`;
    if (ax >= 1e6)
        return `${stripFixedTrailingZeros((x / 1e6).toFixed(2))}M`;
    if (ax >= 1e3)
        return `${stripFixedTrailingZeros((x / 1e3).toFixed(2))}K`;
    return stripFixedTrailingZeros(x.toFixed(2));
}
/** ISO → `MM-DD HH:mm`，省年份与秒 */
function compactPromptUtcFromIso(timeIso) {
...
const PROMPT_RECENT_KLINE_BASE_HEADERS = ["UTC", "O", "H", "L", "C"];
/** 触发 K 线表：标的 + 周期 + OHLC；Vol 仅当策略勾选 {@link StrategyIndicatorId} `VOL` */
function promptTriggerKlineHeaders(orderedIds) {
...
function promptRecentKlineColumnGlossary(orderedIds) {
    ...
    for (const id of STRATEGY_INDICATOR_ORDER) {
        ...
        if (id === "VOL")
            ...
            parts.push("E20=EMA(20,收盘)");
        else if (id === "EM50")
            parts.push("E50=EMA(50,收盘)");
        else if (id === "EM200")
            parts.push("E200=EMA(200,收盘)");
        else if (id === "BB")
            parts.push("BBM/BBU/BBL=布林中轨/上轨/下轨(20周期,2σ)");
        else if (id === "ATR")
            parts.push("ATR=ATR(14,Wilder)");
        else if (id === "RSI14")
            parts.push("RSI=RSI(14,Wilder)");
        else if (id === "MACD")
            parts.push("DIF/SIG/Hist=MACD线/信号线/柱状图(12,26,9)");
        else if (id === "SUPERTREND")
            parts.push("ST/Dir=SuperTrend 线价与方向 U 多 D 空（ATR10×3）");
    }
    return `列说明：${parts.join("；")}。`;
}
 ...
 */
function buildRecentCandlesMarkdownSection(recent, heading = "### 最近 K 线（OKX REST）", strategyIndicators, opts) {
    if (!recent)
        return "";
    const title = heading;
    if (!recent.ok) {
    ...
    const meta = recent.instId && recent.bar
        ? `共 30 根`
        : "";
    ...
    const orderedIds = orderStrategyIndicatorsForPrompt(strategyIndicators === undefined ? ["EM20"] : strategyIndicators);
    ...
    const bbSafe = bb ?? { mid: [], upper: [], lower: [] };
    const tableRows = sliceRows.map((r, j) => {
        ...
        const extra = indicatorCellsForRow(orderedIds, j, i, r.volume, sliceEmaById, bbSafe, atrSeries, rsiSeries, macdTriple, stLine, stUp);
        return [...base, ...extra];
    });
...
function buildMultiTimeframeUserPrompt(symbol, periodKey, candle, recentCandlesByInterval, marketTimeframes, strategyIndicators) {
    ...
    const sections = specs
        .map((spec) => buildRecentCandlesMarkdownSection(recentCandlesByInterval?.[spec.interval], `### ${spec.label} `, strategyIndicators, { omitColumnGlossary: true }))
        .filter(Boolean);
    const klineHead = [
        "## 本次触发的决策 K 线",
        "",
...
async function runTradingAgentTurn(messages, options, agentOpts) {
    ...
    try {
        for (let step = 1; step <= maxSteps; step++) {
            ...
            const completion = await withLlmRetries(cfg, async () => {
                const signal = llmAbortSignal(cfg);
                return client.chat.completions.create({ ...built.body, tools, tool_choice: "auto" }, { signal });
            }, { onRetry });
            ...
            if (wantReasoning) {
                ...
                if (rs)
                    reasoningAcc = mergeReasoningChunk(reasoningAcc, rs);
            }
            thread.push(msg);
...
export { callOpenAIChat, streamOpenAIChat, summarizeAgentAnalysisForCard, buildCardSummarySourceFromAgentThread, runTradingAgentTurn, buildUserPrompt, buildMultiTimeframeUserPrompt, computeEmaSeries, RECENT_CANDLES_FETCH_LIMIT, RECENT_CANDLES_DISPLAY_COUNT, EMA20_PERIOD, EMA50_PERIOD, EMA200_PERIOD, mdCell, mdTable, buildMultimodalUserContent, buildUserTextForHistory, buildChatCompletionRequestFromMessages, resolveSystemPrompt, resolveTradingAgentSystemPrompt, composeSystemPrompt, buildTradingAgentStaticToolsAppend, isLlmEnabled, resolveOpenAiApiKey, keepOnlyLastUserImageInMessages, };
