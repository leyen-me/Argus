...
function createOkxClient(opts) {
    ...
    function request(method, requestPath, bodyStr = "") {
        ...
        if (simulated)
            reqOpts.headers["x-simulated-trading"] = "1";
...
async function fetchSwapInstrument(instId) {
    ...
    const row = j.data?.[0];
    if (!row)
        throw new Error(`未找到 SWAP 合约 ${instId}`);
...
async function fetchOpenSwapPositionsAll(client) {
    ...
    const out = [];
    for (const r of rows) {
        if (swapPositionRowAbsContracts(r) <= 0)
            continue;
...
async function fetchSwapPositionRows(client, instId) {
    const j = (await client.request("GET", `/api/v5/account/positions?instType=SWAP&instId=${encodeURIComponent(instId)}`, ""));
...
async function fetchSwapOrderRow(client, instId, ordId) {
    const j = (await client.request("GET", `/api/v5/trade/order?instId=${encodeURIComponent(instId)}&ordId=${encodeURIComponent(ordId)}`, ""));
    const row = j.data?.[0];
    return (typeof row === "object" && row ? row : null) ?? null;
}
/**
...
async function waitSwapOrderFilled(client, instId, ordId) {
    ...
    for (let i = 0; i < maxTry; i++) {
        ...
        if (terminalBad.has(st)) {
            ...
            throw new Error(`OKX 订单未成交即结束：state=${st} accFillSz=${last.accFillSz ?? ""}${why ? ` ${why}` : ""}。`);
        }
        await delay(stepMs);
    }
    throw new Error(`OKX 订单在 ${(maxTry * stepMs) / 1000}s 内未变为已成交：最后 state=${last?.state} accFillSz=${last?.accFillSz ?? ""}。请用网页或 GET /api/v5/trade/order 核对订单状态。`);
}
/**
 * @param {ReturnType<createOkxClient>} client
 * @param {string} instId
...
async function fetchSwapPositionDetail(client, instId, cachedRows = null) {
    ...
    const { detail, row } = pickBestSwapPositionRowData(rows);
    if (!row || row.mgnMode == null)
        return detail;
    const raw = String(row.mgnMode).toLowerCase();
    if (raw === "cross")
        return { ...detail, mgnMode: "cross" };
    if (raw === "isolated")
        return { ...detail, mgnMode: "isolated" };
    return detail;
}
/**
...
function resolveOpenRiskParams(cfg, args) {
    ...
    if (!Number.isFinite(lever) || lever < 1)
        lever = DEFAULT_OKX_SWAP_LEVERAGE;
    ...
    if (!Number.isFinite(marginFrac) || marginFrac <= 0)
        marginFrac = DEFAULT_OKX_SWAP_MARGIN_FRACTION;
...
function floorToLot(sz, lotSz) {
    if (!Number.isFinite(sz) || !Number.isFinite(lotSz) || lotSz <= 0)
        return 0;
...
function estimateAgentOpenContracts(avail, marginFrac, lever, inst, px) {
    if (!Number.isFinite(avail) || avail <= 0 || !Number.isFinite(px) || px <= 0)
        return 0;
    if (!Number.isFinite(marginFrac) || marginFrac <= 0 || !Number.isFinite(lever) || lever < 1)
        return 0;
...
function resolveCloseParams(posMode, detail) {
    ...
    if (posMode === "hedge") {
        if (ps === "long")
            return { closeSide: "sell", orderPosSide: "long" };
        if (ps === "short")
            return { closeSide: "buy", orderPosSide: "short" };
    }
    /** 单向(net)：下单 API 不传 posSide；由 buy/sell + reduceOnly 表达方向 */
    if (detail.posNum > 0)
        return { closeSide: "sell", orderPosSide: null };
    if (detail.posNum < 0)
        return { closeSide: "buy", orderPosSide: null };
    return { closeSide: "sell", orderPosSide: null };
}
/**
...
function validateAgentOpenAgainstPositions(rows, posMode, wantLong) {
    ...
    if (posMode === "hedge") {
        ...
        const hasTarget = list.some((r) => String(r.posSide || "").toLowerCase() === targetSide && swapPositionRowAbsContracts(r) > 0);
        const hasOpp = list.some((r) => String(r.posSide || "").toLowerCase() === oppSide && swapPositionRowAbsContracts(r) > 0);
        if (hasOpp && !hasTarget) {
            return {
                ok: false,
...
async function placeSwapMarketPosSideFallback(client, base, attempts) {
    ...
    for (let i = 0; i < attempts.length; i++) {
        ...
        try {
            ...
            last = e;
            if (i === attempts.length - 1 || !isOkx51000PosSide(e))
                throw e;
        }
    }
 ...
 */
const OKX_51010_ACCOUNT_MODE_HINT = "【51010】当前交易账户模式不支持该操作。请到 OKX 网页或 App：交易设置 / 账户模式，切换为支持合约与杠杆的模式（如「多币种保证金」「合约模式」等，以界面为准）；模拟盘请在「模拟交易」环境内检查是否已开通合约。也可尝试环境变量 OKX_TD_MODE=isolated。";
/**
...
async function smokeSwapOpenLong(opts) {
    const { apiKey, secretKey, passphrase, simulated = true, instId = "BTC-USDT-SWAP", tdMode = "isolated", lever = 10, } = opts;
    if (!apiKey || !secretKey || !passphrase) {
    ...
    const client = createOkxClient({ apiKey, secretKey, passphrase, simulated });
    if (simulated) {
        console.warn("[Argus/OKX] 当前为模拟盘（x-simulated-trading）。市价单通常瞬间成交，「当前委托」里常为 0 条，请到「订单历史/成交记录」里按时间或 ordId 查；主站实盘合约页看不到。");
    }
    ...
    try {
        ...
        if (isOkx51010AccountMode(e)) {
            console.warn("[Argus] OKX set-leverage 返回 51010，已跳过设杠杆并继续尝试下单（使用账户当前杠杆）。", OKX_51010_ACCOUNT_MODE_HINT);
        }
        else {
            throw e;
        }
    ...
    const openOb = openBody;
    const openD0 = Array.isArray(openOb.data) ? openOb.data[0] : undefined;
...
async function smokeSwapClosePosition(opts) {
    const { apiKey, secretKey, passphrase, simulated = true, instId = "BTC-USDT-SWAP", tdMode = "isolated", pxType: pxTypeRaw, openAccFillSz, } = opts;
    ...
    if (detail.absPos <= 0 && openAccFillSz != null && String(openAccFillSz).trim() !== "") {
        ...
        if (Number.isFinite(accFill) && accFill > 0) {
            ...
            console.warn("[Argus/OKX] GET /account/positions 未返回持仓，使用 openAccFillSz 平仓（多见于模拟盘）。");
            detail =
                posMode === "hedge"
    ...
    const closeOb = closeBody;
    const closeD0 = Array.isArray(closeOb.data) ? closeOb.data[0] : undefined;
...
async function fetchSwapPendingOrders(client, instId) {
    const j = await client.request("GET", `/api/v5/trade/orders-pending?instType=SWAP&instId=${encodeURIComponent(instId)}`, "");
...
async function amendSwapOrder(client, p) {
    ...
    if (p.newPx != null && String(p.newPx).trim() !== "")
        bodyObj.newPx = String(p.newPx);
    if (p.newSz != null && String(p.newSz).trim() !== "")
        bodyObj.newSz = String(p.newSz);
...
async function amendSwapAlgoOrder(client, p) {
    ...
    if (p.algoId != null && String(p.algoId).trim() !== "")
        bodyObj.algoId = String(p.algoId);
    if (p.algoClOrdId != null && String(p.algoClOrdId).trim() !== "")
        bodyObj.algoClOrdId = String(p.algoClOrdId);
    ...
    if (basePxType !== "mark" && basePxType !== "index")
        basePxType = "last";
...
function isOkxExchangeContextReadyForBarAgent(exchangeCtx) {
    if (!exchangeCtx || exchangeCtx.enabled !== true || exchangeCtx.ok !== true)
        return false;
    const pos = exchangeCtx.position;
    if (!pos || typeof pos !== "object")
        return false;
    if (typeof pos.instId !== "string" || !pos.instId.trim())
        return false;
    ...
    if (!cs || typeof cs !== "object")
        return false;
    if (!Number.isFinite(cs.ct_val) || !Number.isFinite(cs.last_px))
        return false;
    return true;
}
/**
...
async function getOkxExchangeContextForBar(cfg, tvSymbol, _interval = "") {
    ...
    try {
        ...
        let sizing_note = "开仓张数由本机按 OKX 规则计算：保证金≈usdt_avail_eq×margin_fraction，名义≈保证金×leverage，张数=floor_step(名义/(ct_val×last_px), lot_sz)，且须≥min_sz；下表为按当前 last_px 的参考值，实际以下单时为准。";
        try {
...
function buildAttachAlgoOrdsForAgentOpen(o) {
    ...
    if (triggerPxType !== "mark" && triggerPxType !== "index")
        triggerPxType = "last";
...
async function executeAgentPerpOpen(cfg, args) {
    const { tvSymbol, side, orderType, limitPrice, barCloseId: _barCloseId, tpTriggerPx, slTriggerPx, tpSlTriggerPxType } = args;
    ...
    const instId = tvSymbolToSwapInstId(tvSymbol);
    if (!instId)
        return { ok: false, message: "无效 OKX 品种" };
    const apiKey = typeof cfg.okxApiKey === "string" ? cfg.okxApiKey.trim() : "";
    const secretKey = typeof cfg.okxSecretKey === "string" ? cfg.okxSecretKey.trim() : "";
    const passphrase = typeof cfg.okxPassphrase === "string" ? cfg.okxPassphrase.trim() : "";
    if (!apiKey || !secretKey || !passphrase) {
    ...
    const addOn = posMode === "hedge"
        ? posRows.some((r) => String(r.posSide || "").toLowerCase() === (isLong ? "long" : "short") &&
            swapPositionRowAbsContracts(r) > 0)
        : existing.absPos > 0;
    const avail = await fetchUsdtAvailEq(client);
    if (avail <= 0)
        return { ok: false, message: "USDT 可用权益为 0" };
    const margin = avail * marginFrac;
    const notional = margin * lever;
    const inst = await fetchSwapInstrument(instId);
    ...
    const ordData = ord.data;
    const ordRow0 = Array.isArray(ordData) ? ordData[0] : undefined;
...
async function executeAgentPerpClose(cfg, args) {
    ...
    const tdMode = detail.mgnMode === "cross"
        ? "cross"
    ...
    const closeData = closeBody.data;
    const closeRow0 = Array.isArray(closeData) ? closeData[0] : undefined;
...
function parseSwapPositionHistoryRealizedPnl(row) {
    ...
    const raw = o.realizedPnl != null && String(o.realizedPnl).trim() !== ""
        ? o.realizedPnl
...
function swapPositionHistoryEffectiveTimeMs(row) {
    if (!row || typeof row !== "object")
        return Number.NaN;
    const o = /** @type {Record<string, unknown>} */ (row);
    const raw = o.uTime != null && String(o.uTime).trim() !== ""
        ? o.uTime
...
async function aggregateSwapClosePositionStats(client, opts = {}) {
    ...
    const maxPagesRaw = typeof opts.maxPages === "number" && Number.isFinite(opts.maxPages) && opts.maxPages > 0
        ? Math.floor(opts.maxPages)
        : 25;
    const maxPages = Math.min(maxPagesRaw, 60);
    const beginMs = opts.beginMs != null && Number.isFinite(Number(opts.beginMs)) && Number(opts.beginMs) > 0
    ...
    const timeRanges = Array.isArray(opts.timeRanges)
        ? opts.timeRanges
            ...
            const endMs = range?.endMs != null && Number.isFinite(Number(range.endMs)) ? Math.floor(Number(range.endMs)) : null;
    ...
    for (let page = 0; page < maxPages; page++) {
        ...
        if (beginMs != null)
            q.set("begin", String(beginMs));
        if (endMs != null && Number.isFinite(endMs) && endMs > 0)
            q.set("end", String(endMs));
        ...
        for (const row of rows) {
            ...
            if (!r || typeof r !== "object")
                continue;
            if (r.instId != null && String(r.instId) !== instId)
                continue;
            const effectiveMs = swapPositionHistoryEffectiveTimeMs(r);
            if (Number.isFinite(effectiveMs))
                oldestMs = Math.min(oldestMs, effectiveMs);
            if (beginMs != null && Number.isFinite(effectiveMs) && effectiveMs < beginMs)
                continue;
            if (!isInTimeRanges(effectiveMs))
                continue;
        ...
        if (!Number.isFinite(oldestMs))
            break;
        if (beginMs != null && oldestMs <= beginMs)
            break;
        endMs = oldestMs - 1;
 ...
 * }>}
 */
async function fetchRecentSwapPositionsHistoryForBar(cfg, tvSymbol, maxRows = 10, sessionSinceMs = null) {
    if (!cfg || cfg.okxSwapTradingEnabled !== true) {
        return { ok: true, skipped: true, reason: "okx_swap_disabled" };
    }
    if (inferFeed(tvSymbol) !== "crypto") {
    ...
    try {
        ...
        if (sinceMsNorm != null)
            q.set("begin", String(sinceMsNorm));
        const json = (await client.request("GET", `/api/v5/account/positions-history?${q.toString()}`, ""));
        if (String(json?.code) !== "0") {
...
export { tvSymbolToSwapInstId, tvIntervalToOkxCandleBar, fetchRecentCandlesForTv, createOkxClient, fetchSwapPendingOrders, fetchSwapPendingAlgoOrders, fetchSwapPendingOrderSummaries, serializePendingSwapOrder, serializePendingSwapAlgoOrder, cancelSwapOrder, amendSwapOrder, amendSwapAlgoOrder, isOkxExchangeContextReadyForBarAgent, describeOkxExchangeContextGateFailure, getOkxExchangeContextForBar, executeAgentPerpOpen, executeAgentPerpPreviewOpen, executeAgentPerpClose, getOkxSwapPositionSnapshot, fetchUsdtAccountMetrics, fetchOpenSwapPositionsAll, fetchTickerLast, fetchSwapInstrument, formatOkxPx, buildAttachAlgoOrdsForAgentOpen, smokeSwapOpenLong, smokeSwapClosePosition, smokeSwapOpenLongThenClose, aggregateSwapClosePositionStats, fetchRecentSwapPositionsHistoryForBar, };
//# sourceMappingURL=okx-perp.js.map