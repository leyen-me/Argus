...
function buildPlainBody(detail) {
    ...
    if (detail.transition)
        lines.push(`转移: ${detail.transition}`);
    if (detail.hardExit) {
        lines.push(`硬触发: ${detail.hardExit.type || ""} side=${detail.hardExit.side || ""} @ ${detail.hardExit.exitPrice ?? ""}`);
    }
    if (detail.prevState)
        lines.push(`之前状态: ${JSON.stringify(detail.prevState)}`);
    if (detail.nextState)
        lines.push(`当前状态: ${JSON.stringify(detail.nextState)}`);
    if (detail.reasoning)
        lines.push(`模型 reasoning: ${detail.reasoning}`);
    return lines.join("\n");
}
/**
 * @param {object} cfg
...
async function notifyTradePositionIfNeeded(cfg, detail) {
    if (!cfg || cfg.tradeNotifyEmailEnabled !== true)
        return;
...
//# sourceMappingURL=trade-notify-email.js.map