...
function createTradingToolExecutor(ctx, deps = TRADING_EXECUTOR_DEFAULT_DEPS) {
    ...
    const { createOkxClient, tvSymbolToSwapInstId, cancelSwapOrder, amendSwapOrder, amendSwapAlgoOrder, executeAgentPerpOpen, executeAgentPerpClose, } = deps;
            ...
            switch (name) {
                case "open_position": {
                    ...
                    if (slTriggerPx === undefined ||
                        ...
                        const warnMsg = "open_position 必须提供 stop_loss_trigger_price；当前已启用强制止损规则。请先明确失效点，再决定是否下单。";
                ...
                case "close_position": {
                    ...
                    const closedSide = ex.preCloseHoldingState === "HOLDING_SHORT"
                        ? "short"
                ...
                case "cancel_order": {
                    ...
                    if (triplet.ok === false)
                        return { ok: false, message: triplet.message };
                    const instId = tvSymbolToSwapInstId(tvSymbol);
                    if (!instId)
                        return { ok: false, message: "无效品种" };
                ...
                case "amend_order": {
                    ...
                    if (a.new_price != null)
                        patch.newPx = a.new_price;
                    if (a.new_size != null)
                        patch.newSz = a.new_size;
                ...
                case "amend_tp_sl": {
                    ...
                    if (newSz === undefined &&
                        ...
                        return {
                            ok: false,
                            message: "amend_tp_sl 至少需要 new_size、take_profit_trigger_price、stop_loss_trigger_price、take_profit_order_price、stop_loss_order_price 之一",
                        };
                    }
                    let tpSlTriggerPxType = String(a.tp_sl_trigger_price_type || "last").toLowerCase();
                    if (tpSlTriggerPxType !== "mark" && tpSlTriggerPxType !== "index") {
                    ...
                    if (newSz !== undefined)
                        patch.newSz = newSz;
                    if (newTpTriggerPx !== undefined)
                        patch.newTpTriggerPx = newTpTriggerPx;
                    if (newSlTriggerPx !== undefined)
                        patch.newSlTriggerPx = newSlTriggerPx;
                    if (newTpOrdPx !== undefined)
                        patch.newTpOrdPx = newTpOrdPx;
                    if (newSlOrdPx !== undefined)
                        patch.newSlOrdPx = newSlOrdPx;
...
//# sourceMappingURL=trading-agent-executor.js.map