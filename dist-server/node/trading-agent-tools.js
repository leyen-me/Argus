...
const TRADING_AGENT_TOOLS = [
    {
        ...
        function: {
            name: "open_position",
            description: "提交开仓单，支持市价和限价。执行层会按当前账户权益、杠杆与 margin_fraction 自动计算张数并校验最小下单量。调用前应完成方向、杠杆、仓位比例与保证金模式判断。限价单提交后可能处于未成交状态；若同时提供止盈止损触发价，主单成交后会由 OKX 创建对应的平仓算法单。",
            parameters: {
                type: "object",
                properties: {
                    side: {
                    ...
                    margin_fraction: {
                        type: "number",
                        description: "占用当前可用 USDT 权益的比例，建议范围 0.01-1。请结合 account_snapshot 控制风险；最终下单张数由程序按交易所规则计算。",
                    },
                    margin_mode: {
                        type: "string",
                        enum: ["isolated", "cross"],
                    ...
                    stop_loss_trigger_price: {
                        type: "number",
                        description: "强制要求，止损触发价。做多时通常应低于入场参考价，做空时通常应高于入场参考价；触发后按市价平仓。若缺少该字段，执行层会拒绝下单。",
                    },
    ...
    },
    {
        ...
        function: {
            name: "close_position",
            description: "平掉当前合约方向的现有持仓，只做 reduce-only，不会反手开仓。支持市价和限价；市价单会等待成交，限价单只提交挂单。",
            parameters: {
                type: "object",
                properties: {
                    order_type: {
    ...
    },
    {
        ...
        function: {
            name: "amend_tp_sl",
            description: "修改当前合约上未触发的止盈止损/条件类算法单（用户快照里的 pending_algo_orders，使用其中的 algoId）。对接 OKX amend-algos。至少提供 new_size、take_profit_trigger_price、stop_loss_trigger_price、take_profit_order_price、stop_loss_order_price 之一。触发价基准默认 last，可与开仓时一致选 mark/index。说明：交易所文档载明不支持移动止损、冰山、TWAP、Trailing 等算法类型；删除止盈/止损腿时可将对应触发价或委托价设为 0（以 OKX 文档为准）。",
            parameters: {
                type: "object",
                properties: {
...
//# sourceMappingURL=trading-agent-tools.js.map