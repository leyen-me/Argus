 ...
 */
async function appendEquitySampleIfNeeded(equityUsdt: number) {
  ...
  const rows = await db
    ...
    .limit(1);
  const last = rows[0] as EquitySampleLastRow | undefined;
  const now = Date.now();
...
async function listRecentEquitySamples(limit = 400) {
  ...
  const rows = await db
    ...
    .orderBy(desc(dashboardEquitySamples.id))
    .limit(n);
  return (rows as EquitySampleRow[])
    .map((r) => ({
      t: String(r.t || ""),
...
async function pruneOldSamples(db: DrizzleDb) {
