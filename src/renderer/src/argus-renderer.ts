...
/**
 * 打开 Agent 会话明细：由 React 弹窗订阅 CustomEvent 拉取数据。
 * @param {string} barCloseId
 */
function openLlmSessionDetailModal(barCloseId) {
  if (!barCloseId) return;
  const root = findLlmRoundRoot(barCloseId);
  const sym = root?.dataset.tvSymbol?.trim() || "";
  const cap = root?.dataset.capturedAt || "";
  window.dispatchEvent(
    new CustomEvent(ARGUS_LLM_SESSION_DETAIL_OPEN, {
      detail: { barCloseId: String(barCloseId), tvSymbol: sym, capturedAt: cap },
    }),
  );
}
